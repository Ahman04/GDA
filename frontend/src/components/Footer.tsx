...
const Footer = () => {
  return (
    <footer className="bg-deep-blue text-deep-blue-foreground pt-16 pb-8">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div>
            ...
            <p className="text-deep-blue-foreground/50 text-sm leading-relaxed mb-6">
              The architect of digital success stories, helping businesses of every size thrive in the online landscape.
            </p>
            <div className="space-y-1.5 text-xs text-deep-blue-foreground/50 mb-6">
            ...
            <div className="flex gap-3">
              {socialLinks.map(({ Icon, hoverClass }, i) => (
                <a
                  key={i}
                  href="#"
                  className={`w-9 h-9 rounded-full bg-deep-blue-foreground/10 flex items-center justify-center transition-colors ${hoverClass}`}
                >
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {["About Us", "Careers", "Case Studies", "Insights", "Contact"].map((l) => (
                <li key={l}>
                  <a href="#" className="text-sm text-deep-blue-foreground/50 hover:text-primary transition-colors">{l}</a>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-deep-blue-foreground/50 mb-4">
              Subscribe for the latest in digital transformation and AI innovation.
            </p>
            <div className="flex gap-2">
              <Input
                placeholder="Your email"
                className="rounded-full bg-deep-blue-foreground/10 border-deep-blue-foreground/10 text-deep-blue-foreground placeholder:text-deep-blue-foreground/30 text-sm h-10"
              />
        ...
        <div className="border-t border-deep-blue-foreground/10 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-deep-blue-foreground/40">
            © {new Date().getFullYear()} Go Digital Africa. All rights reserved.
          </p>
          <div className="flex gap-6">
            {["Privacy Policy", "Terms of Service", "Cookie Policy"].map((l) => (
              <a key={l} href="#" className="text-xs text-deep-blue-foreground/40 hover:text-primary transition-colors">{l}</a>
            ))}
          </div>
        </div>
      </div>
